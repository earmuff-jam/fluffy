...
const AddPlan: React.FunctionComponent<IAddPlanProps> = (props) => {

  const [location, setLocation] = React.useState();
  const [planColor, setPlanColor] = React.useState('#f7f7f7');
  const [status, setStatus] = React.useState(STATUS_OPTIONS[0].label);

  const [planType, setPlanType] = React.useState(ITEM_TYPE_MAPPER['daily'].value);
