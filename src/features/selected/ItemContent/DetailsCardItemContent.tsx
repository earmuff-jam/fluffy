...
interface IDetailsCardItemContentProps {
  selectedItem: SelectedAssetType;
  categoryMode: boolean;
  favBtnDataTour: string;
}

const DetailsCardItemContent: React.FunctionComponent<
  IDetailsCardItemContentProps
> = ({ selectedItem, categoryMode, favBtnDataTour }) => {
  const favItems = [];
  // const { favItems = [] } = useSelector((state) => state.profile);

  const isFavourite = favItems.some(
    (v) =>
      v.category_id === selectedItem.id ||
      v.maintenance_plan_id === selectedItem.id
  );
  ...
  const handleFavItem = (_, selectedID, isFavourite) => {
    ...
    if (isFavourite) {
      ...
      const currentItems = favItems?.filter(
        (v) =>
          v.category_id === selectedID || v.maintenance_plan_id === selectedID
      );
    ...
    <CardContent>
      <Stack direction="row" alignItems="flex-start">
        ...
        >
          <FavoriteRounded
            fontSize="small"
            sx={{ color: isFavourite ? selectedItem.color : "secondary.main" }}
          />
        </IconButton>
...
export default DetailsCardItemContent;
