...
import { AssetSummaryType } from '@features/overview/OverviewHeader/types';

const Overview: React.FunctionComponent = () => {
  const assetSummary = {
    AssetList: [] as AssetSummaryType[],
    AssetSummaryList: [],
  };
  const loading = false;

  const fetchAssignedAssets = (assets: Array<AssetSummaryType> = [], filterParam: string): AssetSummaryType[] => {
    // assets within filterParam must be present
    return assets?.filter((asset) => asset.type.toUpperCase() === filterParam && asset.items[0] != '');
  };
